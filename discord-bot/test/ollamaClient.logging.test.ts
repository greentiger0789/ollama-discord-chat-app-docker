...
import { executeSearchWithDeps } from '../src/ollamaClient.ts';
...
function captureInfoLogs(): unknown[][] {
...
describe('ollamaClient logging', () => {
    test('should log when Tavily is selected and called', async () => {
        ...
        assert.ok(selectionLog && typeof selectionLog[1] === 'object' && selectionLog[1]);
        assert.equal(
            'scope' in selectionLog[1] ? selectionLog[1].scope : undefined,
            'ollamaClient'
        );
        assert.equal(
            'query' in selectionLog[1] ? selectionLog[1].query : undefined,
            'latest topic'
        );
    });
    ...
    test('should log when DuckDuckGo is selected and called', async () => {
        ...
        assert.ok(callLog && typeof callLog[1] === 'object' && callLog[1]);
        assert.equal('scope' in callLog[1] ? callLog[1].scope : undefined, 'ollamaClient');
        assert.equal('query' in callLog[1] ? callLog[1].query : undefined, 'general topic');
    });
});
