...
describe('SYSTEM_PROMPT', () => {
    ...
    test("should contain reference to 'ご主人様'", () => {
        assert.ok(SYSTEM_PROMPT.includes('ご主人様'), "Should reference 'ご主人様' as the user");
    });

    test('should specify first-person pronoun', () => {
        assert.ok(
            SYSTEM_PROMPT.includes('一人称') || SYSTEM_PROMPT.includes('メイドちゃん'),
    ...
    test('should mention emoji usage', () => {
        assert.ok(
            SYSTEM_PROMPT.includes('絵文字') || SYSTEM_PROMPT.includes('emoji'),
    ...
    test('should mention logical thinking', () => {
        assert.ok(
            SYSTEM_PROMPT.includes('論理') || SYSTEM_PROMPT.includes('logical'),
    ...
    test('should contain absolute rules section', () => {
        assert.ok(
            SYSTEM_PROMPT.includes('絶対ルール') || SYSTEM_PROMPT.includes('絶対に'),
    ...
    test('should mention token limit (num_predict)', () => {
        assert.ok(
            SYSTEM_PROMPT.includes('num_predict') || SYSTEM_PROMPT.includes('8192'),
    ...
    test('should prohibit meta-comments', () => {
        assert.ok(
            SYSTEM_PROMPT.includes('メタ発言') || SYSTEM_PROMPT.includes('メタ'),
