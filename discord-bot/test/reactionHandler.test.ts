...
import {
    createHandleReactionAdd,
    type ReactionGeneration,
    type ReactionLike
} from '../src/handlers/reactionHandler.ts';

function createReaction(
    emoji: string,
    options: { partial?: boolean; message?: ReactionLike['message'] } = {}
): ReactionLike {
    const { partial = false, message } = options;
    return {
        emoji: { name: emoji },
        partial,
...
test('reaction handler aborts a matching active generation once', async () => {
    ...
    const entry: ReactionGeneration = { state: 'generating', controller, userId: 'owner' };
    let available: ReactionGeneration | null = entry;
    const handle = createHandleReactionAdd({
...
test('reaction handler regenerates for the owner and does not allow other users', async () => {
    const controller = new AbortController();
    let regenerated = 0;
    const entry: ReactionGeneration = {
        state: 'completed',
        controller,
        userId: 'owner',
        regenerate: async () => {
