...
import type { CommandReply } from '../src/commands/resetCommand.ts';
import type { SummaryInteraction } from '../src/commands/summaryCommand.ts';
...
describe('summaryCommand', () => {
    describe('handleOSummaryCommand structure', () => {
        test('should export handleOSummaryCommand function', async () => {
            const { handleOSummaryCommand } = await import('../src/commands/summaryCommand.ts');
        ...
        test('should export createHandleOSummaryCommand factory', async () => {
            const { createHandleOSummaryCommand } = await import(
                '../src/commands/summaryCommand.ts'
            );
    ...
    describe('outside thread', () => {
        test('should reply ephemeral error and not call summarizeConversation', async () => {
            ...
            const mockInteraction: SummaryInteraction = {
                channel: {
                    isThread: () => false,
                    id: 'channel-1',
    ...
    describe('inside thread with history', () => {
        test('should pass history to summarizeConversation and result to sendSplitMessage', async () => {
            ...
            let capturedHistory: readonly HistoryEntry[] | null = null;
            ...
            const mockInteraction: SummaryInteraction = {
                channel: {
                    isThread: () => true,
                    id: 'thread-123',
                    send: async () => ({ id: 'thinking-msg', edit: async () => {} })
                },
                user: { id: 'user-1' },
                reply: async () => {},
                deferReply: async () => {
    ...
    describe('inside an unmanaged thread', () => {
        test('should reject the command without reading or summarizing history', async () => {
            ...
            const calls: Array<{
                type: string;
                channel?: unknown;
                options?: CommandReply | { clientId?: string | undefined } | undefined;
            }> = [];
            const interaction: SummaryInteraction = {
            ...
            await createHandleOSummaryCommand({
                ...
                getThreadHistory: () => {
                    calls.push({ type: 'getHistory' });
                    return [];
                },
                summarizeConversation: async () => {
                    calls.push({ type: 'summarize' });
                    return '';
                }
            })(interaction);
            ...
            assertDefined(calls[1]);
            assert.equal(calls[1].type, 'reply');
            assert.ok(calls[1].options && 'ephemeral' in calls[1].options);
            assert.equal(calls[1].options.ephemeral, true);
            assert.ok('content' in calls[1].options);
