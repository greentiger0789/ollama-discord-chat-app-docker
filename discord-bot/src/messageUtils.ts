function getRandom<T>(arr: readonly T[]): T {
    ...
    if (value === undefined) {
        throw new Error('Cannot select a value from an empty array.');
    }
    return value;
}
...
export function formatQuotedReference(refMessage: ReferencedMessage): string {
...
export function buildMaidThinkingMessage(): string {
    const emojis = ['☕', '🫖', '🧠', '📡', '🔎', '⚙️', '💭', '📚', '✨', '💻'];
...
export async function sendSplitMessage(
    ...
    text: string,
    firstMessageToEdit: EditableMessage | null = null
): Promise<void> {
    const limit = 1900;
