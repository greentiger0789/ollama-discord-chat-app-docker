...
- **`discord-bot/`**: Node.js 26 の標準 TypeScript 型消去で `.ts` を直接実行する ESM Discord ボット本体。依存は `discord.js` v14、`node-fetch`、`js-yaml`、`@tavily/core`
...
Makefile を推奨（ホスト側で実行）。ボットコンテナ内では自動的に TypeScript 関連ターゲットのみに切り替わる。
...
- **ロガー**: `console.log` ではなく `src/logger.ts` の `createLogger(scope)` を使用（`LOG_LEVEL` 対応、テスト環境では自動的に warn 以上に抑制される）
...
- **Discord 制限**: 応答は 1900 文字で自動分割送信（`messageUtils.ts`）。この上限を変更する場合は Discord API 制限 (2000 文字) を考慮
- **トークン概算**: 日本語 LLM 向けの概算ロジックと履歴要約（最大 12000 トークン）が `ollamaClient.ts` にある。コンテキスト長関連の変更は `models.yml` の `num_ctx` との整合を保つこと
